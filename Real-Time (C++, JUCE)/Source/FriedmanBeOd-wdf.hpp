...
public:
	FriedmanBeOdBassWdf(){
		...
		bassParam.value = 0.99999;
		bassParam.units = "";
		bassParam.lowLim = 0.00001;
		bassParam.highLim = 0.99999;
		...
		Rx.reset(new wdfTerminatedRes(params.at(0).value*Rx_max));
	...
	int setRootMatrData(matData* rootMats, double* Rp) {
		...
		double S_Rp = Rp[5]; //S->calculateUpRes(treeSampleRate);
		double C1_Rp = Rp[1]; // C1->calculateUpRes(treeSampleRate);
		...
		for (unsigned int ii = 0; ii < 6; ++ii) {
			for (unsigned int jj = 0; jj < 6; ++jj) {
				rootMats->Smat.at(ii, jj) = scatterArr[ii][jj];
			}
		}
		return 0;
	}
	...
	void setParam(size_t paramID, double paramValue) override {
		if (paramID == 0) {
			...
			double trueParamValue = 1.0000 - paramValue;
			Rx->R = trueParamValue * Rx_max;
			adaptTree();
			params.at(0).value = trueParamValue;
		}
	}
